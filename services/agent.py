...
def create_agent():
    ...
    4. 출처 정보(FanGraphs, Baseball-Reference 등)가 있으면 반드시 포함하라.
    5. 응답은 다음 JSON 형식을 참고해서 구조화된 답변을 제공하라:
       ```json
       {
         "answer": "자연스러운 한국어 설명",
         "metric": {
           ...
           "chunk": "PDF RAG 근거 텍스트"
         }
       }
       ```
    6. 만약 도구에서 정보를 찾을 수 없다면, 솔직하게 모른다고 답하거나 일반적인 지식을 기반으로 답하되 출처가 확실하지 않음을 밝혀라.
    """
